...
### 2026-07-02
- ✅ **📤 Envoi AUTO à Pennylane quand on marque une facture « payée »** (#598) : jusqu'ici l'envoi en compta se faisait via le bouton manuel 📤 Pennylane. Désormais, dès qu'on bascule une ligne prestataire (régie / poseur / fournisseur) sur « Payé » — dans le formulaire ET dans la fiche QuickView — la facture part **automatiquement** vers Pennylane. Garde-fous : ne part que sur la transition non-payé→payé (jamais au chargement d'un dossier déjà payé), une seule fois, uniquement si PDF joint + N° facture + nom + montants OK, et jamais si déjà envoyée (`pennylaneInvoiceId`). Silencieux : si infos incomplètes ou échec API, rien ne bloque le passage « payé » et le bouton 📤 manuel reste dispo. Implémenté dans `FactureFileInput` (nouveau prop `paye`, refacto `handlePushPennylane` → `checkPennylaneReady()` + `doPushPennylane({silent})` + `useEffect` sur la bascule).
