...
### 2026-07-02
- ✅ **📊 Export comptable CIBLÉ + 💸 ZIP des factures payées** : le bouton « Export comptable » ouvre désormais un modal de choix — « Tout exporter » (comportement historique) OU cocher des tiers précis (poseurs / régies / fournisseurs / internes, recherche + tout cocher par catégorie) → grand livre limité à la sélection (`grand-livre_<soc>_selection_*.csv`). + bouton « 💸 Factures des paiements (ZIP) » : télécharge en un ZIP les factures PDF des lignes marquées « payées » sur la période (7/30/60/90 j ou tout), classées par dossier `Catégorie_Nom/date_client_n°facture.pdf`, limité aux tiers cochés si sélection. Dép. `jszip` (import dynamique). `exportComptable(selectedKeys)` filtre dans `ajoute()`.
