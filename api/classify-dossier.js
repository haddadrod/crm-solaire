...
// ── ANTI-FRAUDE (anciennement /api/fraud-check, fusionné ici pour rester
//    sous la limite de 12 functions du plan Vercel Hobby) ──────────────────
//
// Analyse anti-fraude APPROFONDIE d'UN seul document (un bulletin de paie,
// un avis d'impôt, un justificatif…). classify-dossier fait une passe rapide
// sur tout le dossier ; ici on reprend chaque document sensible séparément
// pour le scruter à fond. Un document = un appel = toute l'attention du
// modèle dessus.
//
// Activé via ?action=fraud sur l'endpoint /api/classify-dossier.
...
export default async function handler(req, res) {
  ...
  // 🔀 Dispatcher : /api/classify-dossier?action=fraud → analyse anti-fraude
  // d'un seul document. Sinon, comportement par défaut : classification +
  // découpe + extraction du dossier complet.
  if (req.query?.action === 'fraud' || req.body?.action === 'fraud') {
    return handleFraudCheck(req, res);
  }
